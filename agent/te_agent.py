...
"""
    ...
    async def _run_nim_loop(self, user_msg: str, tools: list, max_iterations: int) -> int:
        ...
        # Nemotron 3 Super (and the Llama-Nemotron Super line) treat
        # ``detailed thinking off`` / ``detailed thinking on`` as a system-
        # prompt control directive that toggles between long chain-of-
        # thought and direct-response modes. We use ``off`` here because
        # ORCA's tool-using loop needs decisive structured output, not a
        # narrated reasoning trace — the gate already enforces authority
        # discipline so we don't need the model to re-derive it each turn.
        # Override-able via NIM_THINKING=on in env if a deployment wants
        # full reasoning back.
        thinking = os.getenv("NIM_THINKING", "off").lower()
        if thinking not in ("on", "off"):
            thinking = "off"
        sys_content = f"detailed thinking {thinking}\n\n{SYSTEM_PROMPT}"
        messages = [
