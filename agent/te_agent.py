...
async def _write_episode(inputs: dict) -> dict:
    ...
    # ── RESOLUTION block (Distiller-targeted) ────────────────────────────────
    # These fields exist so the Intent Distiller agent can pattern-match an
    # episode → reusable policy intent without re-deriving from prose. All
    # values are passed in deterministically by the caller (stream_approval
    # for Act-1 saves, scenarios._play for Act-2 vendor escalations); the
    # Distiller never has to LLM-paraphrase them.
    #
    # outcome enum (must match Distiller action.type vocabulary verbatim):
    #   save_via_config_change | vendor_escalation | reroute_lsp
    #   adjust_igp_metric | throttle_slice | notify_customer
    #   escalate_to_human | no_action_observe
    customers       = inputs.get("customers", [])
    customers_yaml  = ("\n".join(f'  - "{c}"' for c in customers)
                      if customers else "  []")
    diagnosis       = inputs.get("diagnosis", "") or inputs.get(
        "trigger_description",
        "Network fault detected by ORCA",
    )
    outcome         = inputs.get("outcome", "save_via_config_change")
    arr_at_risk     = inputs.get("arr_at_risk", 0)
    saved_arr       = inputs.get("saved_arr", 0)
    nokia_cfg_chg   = bool(inputs.get("nokia_config_change",
                                       outcome == "save_via_config_change"))
    human_invol     = bool(inputs.get("human_involvement", False))
    scenario_id_val = inputs.get("scenario_id", "")
    affected_count  = inputs.get("affected_customer_count", len(customers))
...
diagnosis: "{diagnosis}"
outcome: "{outcome}"
nokia_config_change: {str(nokia_cfg_chg).lower()}
human_involvement: {str(human_invol).lower()}
customers:
{customers_yaml}
...
decision:
  ...
  reasoning_summary: "{inputs.get('reasoning_summary', 'ORCA analyzed the fault and executed the planned remediation.')}"
  actions_taken:
...
class ORCAAgent:
    def __init__(self, adapter: NetworkAdapter = None, anthropic_api_key: str = None):
        ...
        # Per-cycle artifact links — populated as open_pull_request and
        # write_episode succeed; notify_ops_team / open_tac_case handlers
        # append a "Related artifacts" footer to outgoing emails so every
        # message references the live PR + episode URL.
        self._cycle_pr_url: str = ""
        self._cycle_pr_number = None
        self._cycle_episode_url: str = ""
        # Set by scenarios._play before agent.analyze() so handlers (and the
        # post-cycle episode emitter) can read the active scenario's id +
        # canonical description + impacted_customers list without depending
        # on scenarios.py module-level state.
        self._active_scenario: dict = {}
        print(f"[ORCA] LLM provider={self.provider} model={self.model}", flush=True)

    def _append_artifacts_footer(self, body: str) -> str:
        """Append a Related Artifacts section with the cycle's live PR +
        episode URLs. Skipped if neither exists yet (early-cycle emails)."""
        if not (self._cycle_pr_url or self._cycle_episode_url):
            return body
        pr_line = (
            f"Config PR:   {self._cycle_pr_url}"
            if self._cycle_pr_url
            else "Config PR:   not applicable — fault upstream of authority, no Nokia-domain change proposed"
        )
        ep_line = (
            f"Episode:     {self._cycle_episode_url}"
            if self._cycle_episode_url
            else "Episode:     pending"
        )
        footer = (
            "\n\n─── RELATED ARTIFACTS ─────────────────────────────\n"
            f"{pr_line}\n"
        ...
        return [
            ...
            {"name": "assess_sla_risk",
             ...
             "input_schema": {"type": "object", "properties": {
                 ...
                 "required": []}},
            {"name": "match_policy_intent",
             "description": "FIRST CHECK at the start of any fault response. Look up the ratified policy library for an intent matching the current fault signature. If a match returns with confidence >= 0.75, you MUST execute the prescribed action.type directly under Mission Guardrail envelope missions-v2 and skip full diagnostic reasoning — the system has already learned this pattern. If null is returned, proceed with normal reasoning. Fault_signature keys must use the allowed signals: slice_p99_latency_us, slice_p99_rising_pct, transport_link_util_pct, upf_qer_status, affected_customer_count, customer_arr_at_risk_usd, core_amf_load_pct, transport_link_status, etc.",
             "input_schema": {"type": "object", "properties": {
                 "fault_signature": {
                     "type": "object",
                     "description": "Flat dict of {signal_name: observed_value} that the policy index will evaluate against intent match conditions."
                 }},
                 "required": ["fault_signature"]}},
    ...
    async def _execute_tool(self, name: str, inputs: dict) -> str:
        ...
        try:
            if name == "get_topology":
                ...
                # Same Issue/Resolution body shape as notify_ops_team — only
                # the subject differs ([TAC P3] NOKIA — ... vs ORCA NOC).
                # Vendor metadata (priority, node, fault) lives in the subject;
                # body stays clean for consistency across the email outbox.
                ...
                body = "\n\n".join(parts)
                body = self._append_artifacts_footer(body)
                subject = f"[TAC {sev}] {vendor.upper()} — {fault_type} on {node}"
                result = send_email(to=to, subject=subject, body=body)
...
"""
            ...
            elif name == "match_policy_intent":
                # L5 closed-loop short-circuit: check ratified policy library
                # for an intent matching this fault signature. On a hit, emit
                # a VISIBLE agent_reasoning event so the demo audience sees the
                # learned-from-prior-episode moment ("matched pol-XXX,
                # executing under guardrail envelope missions-v2 — reasoning
                # skipped"). On a miss, the agent proceeds with normal
                # diagnostic reasoning — no short-circuit.
                from agent import policy_index as _pi
                fs = inputs.get("fault_signature") or {}
                if not isinstance(fs, dict):
                    fs = {}
                hit = _pi.match(fs, min_confidence=0.75)
                if hit:
                    intent_id  = hit.get("intent_id", "")
                    action     = hit.get("action") or {}
                    act_type   = action.get("type", "")
                    confidence = hit.get("confidence", 0)
                    # Visible reasoning marker for the audience — the L5 win.
                    await self._emit("agent_reasoning", {"text":
                        f"📚 Matched {intent_id} from prior episode — "
                        f"executing prescribed action `{act_type}` directly "
                        f"under guardrail envelope missions-v2. "
                        f"Reasoning skipped (confidence {confidence})."
                    })
                    result = {
                        "matched":     True,
                        "intent_id":   intent_id,
                        ...
                        "action":      action,
                        "source_episode": hit.get("source_episode", ""),
                        "instruction": (
                            f"Execute action.type={act_type} directly. "
                            f"Do NOT run diagnostic reads. Use action.parameters "
                            f"verbatim. If action.type=vendor_escalation, call "
                            f"open_tac_case with the parameters. If action.type=save_via_config_change, "
                            f"call propose_config_change. After executing, stop."
                        ),
                    }
                else:
                    result = {
                        "matched":     False,
                        "instruction": "No matching ratified policy intent. "
                                       "Proceed with full diagnostic reasoning.",
                    }
                ...
                enriched.setdefault("trigger_type", "unknown")
                ...
                # GROUND-TRUTH OVERRIDE (safe): only when a scenario is
                # active. _active_scenario is set at scenario start and
                # cleared at scenario end, so this is empty (skipped) for
                # the agent's free-running incidents.
                _gt = getattr(self, "_active_scenario", {}) or {}
                if _gt.get("id"):
                    enriched["scenario_id"]             = _gt["id"]
                    enriched["customers"]               = list(_gt.get("impacted_customers", []))
                    enriched["affected_customer_count"] = len(_gt.get("impacted_customers", []))
                    if _gt.get("trigger_type"):
                        enriched["trigger_type"] = _gt["trigger_type"]
                    if _gt.get("arr_at_risk"):
                        enriched["arr_at_risk"] = _gt["arr_at_risk"]
                result = await _write_episode(enriched)
                # Cache the episode URL on the cycle so subsequent emails
                # (notify_ops_team, open_tac_case, the deploy email) can
                # link to the specific YAML rather than just the folder.
                if result.get("success") and result.get("url"):
                    self._cycle_episode_url = result["url"]
                ...
                if result.get("success") and result.get("pr_url"):
                    for p in reversed(_config_proposals):
                        if p["status"] in ("pending", "saved"):
                            ...
                            break
                    # Cache on the cycle so any later notify/TAC emails
                    # carry the live PR link in their Related Artifacts footer.
                    self._cycle_pr_url = result["pr_url"]
                    self._cycle_pr_number = result["pr_number"]
                ...
                max_b = max(util_before.values()) if util_before else 0
                max_a = max(util_after.values())  if util_after  else 0
                # Snapshot the active scenario so stream_approval can derive
                # the episode's canonical `diagnosis` (= scenario.description)
                # and `customers` list deterministically — no LLM rewording.
                _scn = getattr(self, "_active_scenario", {}) or {}
                proposal = {
                    ...
                    "scenario_id":          _scn.get("id", ""),
                    "scenario_description": _scn.get("description", ""),
                    "impacted_customers":   list(_scn.get("impacted_customers", [])),
                }
                _config_proposals.append(proposal)

                # Auto-fire the NOC + TAC pre-approval emails INLINE from
                # the proposal data. The previous design had the LLM call
                # notify_ops_team and open_tac_case as separate iterations,
                # each costing 5-15s on low_effort — so the emails landed
                # 20-30s AFTER the proposal popped up in the dashboard.
                # Now both emails are generated from the proposal's own
                # title/reason/id text in the SAME iteration. They land
                # simultaneously with the proposal. Trigger-type drives
                # the TAC subject (Act 1 -> Nokia, transport faults are
                # handled by Act 2's manual open_tac_case to Juniper).
                pid          = proposal["id"]
                title_text   = proposal.get("title", "Config change proposed")
                reason_text  = (proposal.get("reason") or "").strip()
                impact_text  = (proposal.get("projected_improvement") or "").strip()
                ts           = datetime.utcnow().strftime("%Y-%m-%d %H:%M UTC")
                # Affected node — used in TAC subject. Pulled from first
                # change's device. Fallback to a generic label.
                changes_list = proposal.get("changes") or []
                first_device = (changes_list[0].get("device") if changes_list else None) or "Nokia core"
                # Severity heuristic: validation any-fail -> critical; else major.
                any_fail     = any(not v for v in validation_checks.values())
                sev          = "critical" if any_fail else "major"
                emoji        = {"critical":"🔴","major":"🟠","minor":"🟡","info":"🟢"}[sev]
                # Issue + Resolution Path body — single template, both emails.
                issue_body   = reason_text or f"{title_text} — see proposal {pid} for details."
                resolution_body = (
                    f"Proposed config change: {title_text}. "
                    f"Proposal {pid} is ready for review in the dashboard; "
                    f"ops team to review and approve."
                    + (f" Projected impact: {impact_text}." if impact_text else "")
                )
                body_parts = [
                    f"1. Issue [{sev.upper()}]: {issue_body}",
                    f"2. Resolution Path: {resolution_body}",
                    f"Proposal ID: {pid}",
                    f"{ts} | by ORCA",
                ]
                body = "\n\n".join(body_parts)
                body = self._append_artifacts_footer(body)
                ops_to = os.getenv("OPS_EMAIL", "sireenmalik@gmail.com")
                noc_subject = f"{emoji} ORCA [{sev.upper()}]: {title_text[:80]}"
                tac_subject = f"[TAC P3] NOKIA — performance_degradation on {first_device}"
                # Both emails fire IMMEDIATELY, same iteration as the
                # proposal — no 20-30s gap on subsequent iters.
                try:
                    send_email(to=ops_to, subject=noc_subject, body=body)
                    send_email(to=ops_to, subject=tac_subject, body=body)
                except Exception:
                    pass  # don't fail proposal creation over email queueing

                result = {"success": True, "proposal_id": proposal["id"],
                          "message": f"Config proposal '{proposal['title']}' ready for review · NOC + TAC notified inline",
                          "emails_sent": 2}
            else:
    ...
    async def analyze(self, context: str = None, restricted_tools: list = None, effort: dict = None) -> dict:
        # Reset per-cycle artifact tracking — each new analyze cycle gets
        # its own PR and episode pair (or none, for gated scenarios).
        self._cycle_pr_url = ""
        self._cycle_pr_number = None
        self._cycle_episode_url = ""
        ...
        # Set the signature BEFORE running so subsequent poll-loop analyzes
        # (context=None) dedup against this in-flight or just-completed cycle.
        # We set on BOTH branches: scenario-driven (context=non-empty) AND
        # poll-driven. Otherwise the poll loop fires its own parallel analyze
        # while the scenario cycle is still working, producing duplicate
        # notify/TAC emails. (User feedback 2026-05-10: 'too many emails'.)
        if fault_signature:
        ...
        if self.provider == "nim":
            iterations = await self._run_nim_loop(user_msg, tools, max_iterations, effort=effort)
        else:
    ...
    async def _run_nim_loop(self, user_msg: str, tools: list, max_iterations: int, effort: dict = None) -> int:
        oa_tools = self._tools_anthropic_to_openai(tools)
        # Reasoning intensity is controlled per-call via Nemotron's extra_body
        # parameters (validated 2026-05-09 against integrate.api.nvidia.com/v1):
        #   reasoning_effort: "none" | "low" | "medium" | "high"
        #     — discrete level. "none" = direct response, "high" = deep CoT.
        #   low_effort: true
        #     — Sonnet-shape light reasoning. ~Sonnet-default behaviour.
        #   reasoning_budget: int
        #     — token budget for chain-of-thought. Pair with max_tokens >= 1500
        #       or content gets truncated (reasoning eats the output budget).
        # Caller passes ``effort`` as a dict; ``max_tokens`` can also live in
        # the dict to override the default. Default = light reasoning.
        eff = dict(effort) if effort else {"chat_template_kwargs": {"enable_thinking": True, "low_effort": True}}
        max_tokens = eff.pop("max_tokens", 8192)
        extra_body = eff or None  # None → don't send the field at all
        if extra_body:
            print(f"[NIM] effort={extra_body} max_tokens={max_tokens}", flush=True)
        messages = [
