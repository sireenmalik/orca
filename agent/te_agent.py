    ...
    async def _execute_tool(self, name: str, inputs: dict) -> str:
        ...
        try:
            if name == "get_topology":
                ...
                # ── Send NOC security email immediately ──────────────────────
...
EVIDENCE ARCHIVED
-----------------
{evidence_url if evidence_url else 'Evidence archival in progress...'}

REVERT PROPOSAL
---------------
A human-gated config revert proposal has been created in the ORCA dashboard.
Approve to restore {node} to last known good baseline.

ACTION REQUIRED
---------------
1. Review evidence PR: {evidence_url or 'see GitHub'}
2. Approve revert proposal in ORCA dashboard → Config Proposals
3. Verify {node} running config after revert

---
Auto-generated by ORCA v32 · Autonomous Network Operations & Response Agent
"""
                send_email(
                    to=ops_email_addr,
                    subject=f"🔐 ORCA SECURITY ALERT [{severity.upper()}]: Unauthorized config on {node} — Evidence archived | Revert pending",
                    body=sec_email_body
                )
